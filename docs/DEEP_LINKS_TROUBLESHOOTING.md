...
### Формат ссылки для публичного доступа
```
https://looking-web.vercel.app/feed?stylist={STYLIST_ID}&look={LOOK_ID}
```

**Важно:** Ссылка должна содержать `/feed` чтобы работать для неавторизованных пользователей!

### Ожидаемое поведение

1. Пользователь открывает ссылку с `/feed`
2. Загружается приложение Looking на странице Feed
3. После загрузки автоматически:
   - Приложение переходит к профилю стилиста
...
3. Выполните:
```javascript
window.location.href = "https://looking-web.vercel.app/feed?stylist=1a89bf8c-bb99-4a9b-8c9d-0d7435a8c1e6&look=4ceec4e3-dfc2-49b9-a5bd-eac0beffd102"
```
4. Приложение должно:
   - Открыть страницу Feed
   - Автоматически перейти к профилю стилиста с образом
...
4. **Проверьте правильный формат:**
Убедитесь, что используете правильный формат с `/feed`:
```
✅ Правильно: https://looking-web.vercel.app/feed?stylist=xxx&look=yyy
❌ Неправильно: https://looking-web.vercel.app?stylist=xxx&look=yyy
```

Формат без `/feed` может не работать для неавторизованных пользователей!
